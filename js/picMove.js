    ...
    $.fn.extend({
        fade: function (opt) {
            ...
            function move() {
                ...
                //图片切换
                $box.children('div').eq($index).stop().fadeIn(1000, function () {
                    if ($flag) {
                        clearTimeout($timer);
                ...
                //焦点切换
                $oUl.children('li').eq($index).stop().fadeTo(1000, 0.9).siblings('li').stop().fadeTo(1000, 0.3);
            }
            //焦点切换
            $oUl.children('li').click(function () {
                    $index = $(this).index();
                    $box.children('div').eq($index).stop().fadeIn(1000).siblings('div').stop().fadeOut(1000);
                    $(this).stop().fadeTo(1000, 0.9).siblings('li').stop().fadeTo(1000, 0.3);
                });
                //左右点击切换
            $box.hover(function () {
                    $flag = false;
                    clearTimeout($timer);
                    $(this).children('h3').stop().fadeIn(1000);
                    ...
                    $flag = true;
                    $timer = setTimeout(move, o.times);
                    $(this).children('h3').stop().fadeOut(1000)
                });
                //阻止文本被选中
